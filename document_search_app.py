...
class FileIndex:
    def __init__(self, folder_path=None):
        ...
        self.index_file = os.path.join(self.app_data_path, 'search_index.json')
        self.index = self.load_index()

    def get_file_hash(self, file_path):
        """מחשב hash של קובץ על בסיס גודל ותאריך שינוי אחרון"""
        if not os.path.exists(file_path):
            return None
        stats = os.stat(file_path)
        return hashlib.md5(f"{stats.st_size}_{stats.st_mtime}".encode()).hexdigest()

    def needs_update(self):
        """בודק אם האינדקס צריך עדכון"""
        ...
        for file_path in list(self.index.keys()):
            if not os.path.exists(file_path):
                del self.index[file_path]
                continue
            
            current_hash = self.get_file_hash(file_path)
            if current_hash != self.index[file_path]['hash']:
                return True
    ...
    def search(self, query, search_options=None):
        """חיפוש בתוך האינדקס עם אפשרויות מתקדמות"""
        if search_options is None:
            search_options = SearchOptions()
        
        results = []
        query_words = query.split()
        exclude_words = [w.lower() for w in search_options.exclude_words]
        ...
        for file_path, file_data in self.index.items():
            ...
            # בדיקת תאריך
            if search_options.date_range['start'] or search_options.date_range['end']:
                file_time = datetime.fromtimestamp(os.path.getmtime(file_path))
                if (search_options.date_range['start'] and file_time < search_options.date_range['start']) or \
                   (search_options.date_range['end'] and file_time > search_options.date_range['end']):
                    continue

            # חיפוש בשם הקובץ ונתיב
            if search_options.search_in_path:
                path_text = f"{file_path} {file_data['filename']}".lower()
                if not all(word.lower() in path_text for word in query_words):
                    continue

            for content_item in file_data['contents']:
                content = content_item['content']
                
                # בדיקת מספר מילים מינימלי
                if search_options.min_word_count:
                    if len(content.split()) < search_options.min_word_count:
                        continue

                # בדיקה שאין מילים להוצאה
                if exclude_words and any(word in content.lower() for word in exclude_words):
                    continue
                ...
                if search_options.exact_match:
                    ...
                    if search_options.word_distance is not None and len(query_words) > 1:
                        # חיפוש עם מרחק בין מילים
                        for i in range(len(words)):
                            if words[i] == query_words[0].lower():
                                # בדיקה אם המילה השנייה נמצאת במרחק המבוקש
                                for j in range(i + 1, min(i + search_options.word_distance + 1, len(words))):
                                    if words[j] == query_words[1].lower():
                                        match_found = True
                                        break
                            if match_found:
                                break
                        ...
                        # חיפוש מדויק רגיל
                        if search_options.match_all_words:
                            match_found = all(any(w == qw.lower() for w in words) for qw in query_words)
                        else:
                            match_found = any(any(w == qw.lower() for w in words) for qw in query_words)
                else:
                    # חיפוש רגיל
                    content_lower = content.lower()
                    if search_options.match_all_words:
                        match_found = all(qw.lower() in content_lower for qw in query_words)
                    else:
                        match_found = any(qw.lower() in content_lower for qw in query_words)

                if match_found:
                    context = self.get_context(content, query_words, search_options, context_length=200)
                    location_info = self.get_location_info(content_item)
...
class AppSettings:
    def __init__(self):
        ...
        self.default_settings = {
            'theme': 'light',
            'primary_color': '#0078D4',  # צבע ברירת מחדל
            'font_size': 'normal',  # small, normal, large
            'indexes': [],  # רשימת האינדקסים
            'selected_indexes': [] 
        }
        self.settings = self.load_settings()
        
        # וידוא שכל המפתחות הנדרשים קיימים
        self.validate_settings()

    def validate_settings(self):
        """וידוא שכל ההגדרות הנדרשות קיימות"""
        was_changed = False
        ...
        if os.path.exists(self.settings_file):
            try:
                with open(self.settings_file, 'r', encoding='utf-8') as f:
                    ...
                    # מיזוג עם הגדרות ברירת המחדל להבטחת קיום כל השדות
...
class DocumentSearchApp:
    def __init__(self, page: Page):
        ...
        self.force_book_path = None  # ברירת מחדל: אין בקשה מיוחדת



        # טען סוגי קבצים ומרחק מילים מהגדרות
    ...
    def toggle_file_filter(self, e):
        ...
        # רענון שורת הכפתורים
        self.advanced_settings.content = self.create_file_type_buttons_row()
    ...
    def create_search_view(self):
        ...
        self.advanced_settings = ft.Container(
            content=self.create_file_type_buttons_row(),
            visible=False,
            padding=10,
            border_radius=8,
                ...
                controls=[
                    ...
                    ft.Container(
                        content=ft.Column([
                            ...
                            ft.TextField(
                                label="מספר מילים מינימלי",
                                value=str(self.app_settings.get_setting('min_word_count', 0)),
                                width=200,
                                keyboard_type=ft.KeyboardType.NUMBER,
                                on_change=lambda e: self.update_search_setting('min_word_count', int(e.control.value or 0))
                            )
                        ]),
                        padding=10
                    )
    ...
    def navigation_change(self, e):
        ...
        if selected_index == 1:
            books = [
                (file_path, file_data)
                for file_path, file_data in self.file_index.index.items()
    ...
    def search_documents(self, e):
        ...
        try:
            ...
            if not selected_indexes:  # אם אין אינדקסים נבחרים, השתמש בכולם
                selected_indexes = [idx['path'] for idx in self.app_settings.settings['indexes']]

            all_results = []
            # חיפוש בכל אינדקס נבחר
            for index_path in selected_indexes:
                self.file_index.folder_path = index_path
                results = self.file_index.search(self.search_term.value, self.search_options)
    ...
    def pick_folder_result(self, e: FilePickerResultEvent):
        """טיפול בבחירת תיקייה"""
        if e.path:
            ...
            self.file_index.folder_path = e.path
            
            try:
                if self.file_index.needs_update():
                    self.show_status("מאנדקס קבצים...")
                    self.index_progress.visible = True
                    self.page.update()
                    
                    self.file_index.update_index(callback=self.update_progress)
                ...
                if self.navigation_bar.selected_index == 1:
                    new_settings_view = self.create_settings_view()
                    self.main_container.content.controls[0].controls[1] = new_settings_view
                    self.settings_view = new_settings_view
    ...
    def remove_index(self, e):
        ...
        # במקום ליצור תצוגה חדשה, נעדכן רק את רשימת האינדקסים
        if self.navigation_bar.selected_index == 1:
            tabs = self.settings_view.content.controls[1]
            index_management = tabs.tabs[0].content
            index_list = index_management.content.controls[-1]  # הרשימה היא הרכיב האחרון
            index_list.controls = self.create_index_list().controls
    ...
    def refresh_index(self, e=None):
        """מרענן את האינדקס"""
        if self.file_index.needs_update():
            self.show_status("מעדכן אינדקס...")
            self.index_progress.visible = True
            self.page.update()
            self.file_index.update_index(callback=self.update_progress)
            self.index_progress.visible = False
            self.show_status("האינדקס עודכן בהצלחה")
        else:
            self.show_status("האינדקס מעודכן")
        self.page.update()


    def create_index_checkbox_list(self):
        """יצירת רשימת תיבות סימון לאינדקסים"""
        checkbox_list = Column(spacing=10)
        
        # מצב הסימון של כל אינדקס נשמר בהגדרות
        if 'selected_indexes' not in self.app_settings.settings:
            self.app_settings.settings['selected_indexes'] = []
            self.app_settings.save_settings()

        for idx in self.app_settings.settings['indexes']:
            is_selected = idx['path'] in self.app_settings.settings['selected_indexes']
        ...
        # אם אין אינדקסים, הצג הודעה מתאימה
    ...
    def create_books_view(self):
        ...
        # בניית רשימת ספרים עם שורה אחת בלבד לכל קובץ
        ...
        # כפתורי דפדוף (רק לשימוש פנימי, לא יוצגו ישירות)
        ...
        # חיצים מעלה/מטה לניווט בין עמודים
        ...
        left_panel = ft.Container(
            content=ft.Column([
                ...
                ft.Container(
                    self.book_page_text,  # <<< כאן השינוי! לא ft.Text חדש, אלא ה־Text שלך
                    expand=True,    # חשוב! העמוד יגדל עם שאר הפאנל
                    padding=20,
                    bgcolor=ft.Colors.WHITE,
                    border_radius=7,
        ...
        # VerticalDivider אינו תומך ב-margin - עוטפים ב-Container
        ...
        # עטוף את הכל ב-Container עם padding חיצוני לאלגנטיות
        ...
        def select_book(file_path):
            file_data = self.file_index.index.get(file_path)
            if not file_data:
